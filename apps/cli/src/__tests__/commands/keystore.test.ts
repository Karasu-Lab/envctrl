...
describe('KeystoreCreateSubCommand', () => {
  ...
  it('creates keystore directory and registers entry', async () => {
    ...
    const { readRegistry, writeRegistry } = await import('../../commands/keystore/registry.js');
    const { KeystoreCreateSubCommand } = await import('../../commands/keystore/create.js');
    ...
    const result = await cmd.execute({ keystorePath: '/custom/ks', name: 'myks' }, mockContext);
  ...
  it('uses default path when none provided', async () => {
    ...
    const cmd = new KeystoreCreateSubCommand();
    const result = await cmd.execute({ keystorePath: undefined, name: undefined }, mockContext);
...
describe('KeystoreListSubCommand', () => {
  ...
  it('returns all registry entries', async () => {
    ...
    const { KeystoreListSubCommand } = await import('../../commands/keystore/list.js');
...
describe('KeystoreDeleteSubCommand', () => {
  ...
  it('rejects deletion without --force', async () => {
    const { readRegistry } = await import('../../commands/keystore/registry.js');
    const { KeystoreDeleteSubCommand } = await import('../../commands/keystore/delete.js');

    vi.mocked(readRegistry).mockResolvedValue([{ name: 'myks', path: '/custom/ks' }]);
